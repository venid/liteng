 ...
 { GLuint attr = 0;
   if(m_init == 0)
    { lock();
      ...
      attr = 3;
      glEnableVertexAttribArray(attr);
      glVertexAttribPointer(attr, 4, GL_FLOAT, false, sizeof(Vertex), BUFFER_OFFSET(sizeof(float) * 8));
      attr = 4;
      glEnableVertexAttribArray(attr);
      glVertexAttribPointer(attr, 3, GL_FLOAT, false, sizeof(Vertex), BUFFER_OFFSET(sizeof(float) * 12));
      attr = 5;
      glEnableVertexAttribArray(attr);
      glVertexAttribPointer(attr, 3, GL_FLOAT, false, sizeof(Vertex), BUFFER_OFFSET(sizeof(float) * 15));
